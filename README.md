...
`frame-ancestors` by design. Revisit only if a CDN/proxy (e.g. Cloudflare) is
ever placed in front. Everything else in the policy (`default-src 'none'`,
`script-src 'self'` + hashes, `style-src 'self'`, etc.) is enforced.

Analytics: none. Phase 1 shipped Cloudflare Web Analytics; it's dropped here to
keep the strict CSP and the "no analytics unless asked" rule. Re-add only with
